...
In addition, the LCPFN framework has a long pipeline which is very well adapted to what it was used for in its corresponding paper[1]. That made it challening to adjust it to a different use. It required a lot of backwards engineering and tweaking to make it work for some of our purposes. The amount of tweaking required is very likely the reason for the subpar performance of the new models trained on the LCDB data.
